from factscore_utils.lm import LM
import openai
import sys
import time
import os
...
class OpenAIModel(LM):
    ...
    def _generate(self, prompt, max_sequence_length=2048, max_output_length=128):
        ...
        if self.model_name == "ChatGPT":
            ...
            return output, response
        elif self.model_name == "TurboInstructGPT":
            # Call API
            response = call_TurboInstructGPT(prompt, temp=self.temp)
            # Get the output from the response
            output = response["choices"][0]["text"]
            return output, response
        else:
...
def call_TurboInstructGPT(prompt, model_name="gpt-3.5-turbo-instruct", max_len=512, temp=0.7, num_log_probs=0, echo=False, verbose=False):
    # call GPT-3 API until result is provided and then return it
    response = None
    received = False
    num_rate_errors = 0
