...
class FactScorer(object):
    ...
    def get_score(self,
        ...
        init_scores = []
        decisions = []
        for topic, generation, sent, facts in zip(topics, generations, sentences, atomic_facts):
            flat_facts = [fact for fact_lst in facts for fact in fact_lst]
            if facts is None or len(flat_facts) == 0:
                decisions.append(None)
            else:
                decision = self._get_score(topic, generation, sent, facts, knowledge_source)
    ...
    def _get_score(self, topic, generation, sentences, atomic_facts, knowledge_source, cost_estimate=None):
        decisions = []
        total_words = 0
        for atoms, sentence in zip(atomic_facts, sentences):
            for atom in atoms:
                atom = atom.strip()
                if self.lm:
                    ...
                    # logging.critical(output[0].lower())
...
if __name__ == '__main__':
    ...
    tot = 0
    topics, generations, atomic_facts, sentences = [], [], [], []
        ...
        for line in f:
            ...
            if args.use_atomic_facts:
                ...
                for sent in dp["annotations"]:
                    ...
                    atoms = [atom["text"] for atom in sent["model-atomic-facts"]]
                    atom_lst.append(atoms)
                sentences.append(sent_lst)
                atomic_facts.append(atom_lst)
            else:
                topics.append(dp["topic"])
                generations.append(dp["output"])
    ...
    with open(args.input_path.replace(".jsonl", f"_factscore_sent_level_output.json"), 'w') as f:
        json.dump(out, f, cls=CustomJSONEncoder)
